...
type Config struct {
	...
	Compute        string
	Version        string
	Target         string // the compute cluster target
	VpcID          string
	SubnetIDs      []string
	SecurityGroups []string

	Existing []string // arn of existing services that might be deployed
}
...
func (p *Provisioner) Read() error {
	...
	services, err := svc.ListServices(&ecs.ListServicesInput{
		Cluster: aws.String(p.Config.Target),
	})
	...
	for _, arn := range services.ServiceArns {
		...
		for _, tag := range tags.Tags {
			t := aws.StringValue(tag.Key)
			v := aws.StringValue(tag.Value)
			if strings.Contains(t, masterTag) || strings.Contains(v, masterTag) {
				p.Config.Existing = append(p.Config.Existing, aws.StringValue(arn))
			}
		}
	}
